    ...
    let handleSubmit = async (e) => {
        ...
        try {
            ...
            if (albumThumbnail) {
                ...
                //! convert album thumbanailn (image) into binary data
                let albumData = new FormData();
                albumData.append("file", albumThumbnail);
                albumData.append("upload_preset", "music_albums");
                albumData.append("cloud_name", "djhe4kv2h");

                //! upload albumthumbnail to cloudrnaiy
                let response = await fetch("https://api.cloudinary.com/v1_1/djhe4kv2h/upload",
                    {
                        method: "POST",
                        body: albumData
                    }
            ...
            await Promise.all(
                songDetails.map(async (song, index) => {
                    ...
                    if (song.songThumbnail) {
                        ...
                        songFileData.append("cloud_name", "djhe4kv2h");
                    ...
                    console.log("song Data Object:", songDataObject)
                    //!Push the song data inside  the song data array
                    songData.push({
                        ...songDataObject,
    ...
    return (
        <section className="w-full flex flex-col justify-center items-center p-6 text-white">
            ...
            <article className="w-full bg-gray-900 rounded-t-md">
                <header>
                    <h1 className="text-3xl text-center uppercase font-bold py-6 text-white">
                        Add Album
                    </h1>
                </header>
            </article>
            <main className="w-full bg-gray-700 rounded-b-md flex-wrap">
